...
import { localTools, callLocalTool } from "./localTools.js";
...
async function getAgentResponse(messages) {
  ...
  const toolsObject = await adoMCPClient.listTools();
  const mcpTools = mcpToolsToOpenAITools(toolsObject.tools);
    ...
    if (message.tool_calls?.length) {
      ...
      const toolResponses = await Promise.all(
        message.tool_calls.map(async (toolCall) => {
          ...
          if (isLocalTool) {
            const output = await callLocalTool(toolCall.function.name, args);
            console.log("Local Tool output:", output);
            return {
              role: "tool",
          ...
          const output = await adoMCPClient.callTool({
